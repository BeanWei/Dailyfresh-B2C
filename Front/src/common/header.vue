        ...
        <div class="w-box">
          <div class="nav-logo">
            <h1 @click="changePage(-1)">
              <router-link to="/" title="天天生鲜">天天生鲜</router-link>
            </h1>
          </div>
          <div class="right-box">
            <div class="nav-list">
                          ...
                          <li class="clearfix" v-for="(item,i) in cartList" :key="i">
                            <div class="cart-item">
                              <div class="cart-item-inner">
                                <a @click="openProduct(item.goods.id)">
                                  <div class="item-thumb">
                                    <img :src="item.goods.image">
                                  </div>
                                  <div class="item-desc">
                                    <div class="cart-cell"><h4>
                                      <a href="" v-text="item.goods.name"></a>
                                    </h4>
                                      <!-- <p class="attrs"><span>白色</span></p> -->
                                      <h6><span class="price-icon">¥</span><span
                                        class="price-num">{{item.goods.price}}</span><span
                                        class="item-num">x {{item.nums}}</span>
                                      </h6></div>
                                  </div>
                                </a>
                                <div class="del-btn del" @click="delGoods(item.goods.id)">删除</div>
                              </div>
                            </div>
                          </li>
                        </ul>
        ...
        <div class="nav-sub" :class="{fixed:st}">
          ...
          <div class="nav-sub-wrapper" :class="{fixed:st}">
            <div class="w">
              <ul class="nav-list2">
                <li>
                  <router-link to="/"><a @click="changeGoods(-1)" :class="{active:choosePage===-1}">首页</a></router-link>
                </li>
                <li v-for="(citem, i) in category_list" :key="i">
                  <a @click="category_goods(citem.id)" :class="{active:choosePage===-2}">{{citem.name}}</a>
                </li>
                <!-- <li v-for="(item,i) in navList" :key="i">
                  <a @click="changeGoods(i, item)" :class="{active:i===choosePage}">{{item.picUrl}}</a>
                </li> -->
              </ul>
              <div></div>
            </div>
...
<script>
  ...
  import { loginOut, categories, indexgoods } from '/api/index'
    ...
    computed: {
      ...
      totalPrice () {
        ...
        this.cartList && this.cartList.forEach(item => {
          totalPrice += (item.nums * Number(item.goods.price))
        })
        return totalPrice
      },
      // 计算数量
