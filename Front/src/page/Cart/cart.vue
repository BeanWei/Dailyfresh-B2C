                ...
                <div class="cart-group divide pr" :data-productid="item.goods.id">
                  <div class="cart-top-items">
                    <div class="cart-items clearfix">
                      ...
                      <span class="blue-checkbox-new " :class="{'checkbox-on': selectedArry.indexOf(item.goods.id) > -1}"
                            @click=selected(item.goods.id)></span>
                      </div>
                      <!--图片-->
                      <div class="items-thumb fl">
                        <img :alt="item.goods.name"
                             :src="item.goods.image">
                        <a @click="goodsDetails(item.goods.id)" :title="item.goods.name" target="_blank"></a>
                      </div>
                      <!--信息-->
                      <div class="name hide-row fl">
                        <div class="name-table">
                          <a @click="goodsDetails(item.goods.id)" :title="item.goods.name" target="_blank"
                             v-text="item.goods.name"></a>
                          <!-- <ul class="attribute">
                            <li>白色</li>
                          </ul> -->
                        </div>
                      ...
                      <!--删除按钮-->
                      <div class="operation">
                        <a class="items-delete-btn" @click="cartdel(item.goods.id)"></a>
                      </div>
                      <!--商品数量-->
                      <div>
                        <!--总价格-->
                        <div class="subtotal" style="font-size: 14px">¥ {{Number(item.goods.price) * item.nums}}</div>
                        <!--数量-->
                        <buy-num :num="item.nums"
                                 ...
                                 :limit="item.goods.stock"
                                 @edit-num="EditNum"
                        >
                        </buy-num>
                        <!--价格-->
                        <div class="price1">¥ {{item.goods.price}}</div>
                      </div>
                    </div>
                  </div>
                </div>
              ...
              <div class="cart-bar-operation">
                <div>
                  <div class="choose-all">
                    <span :class="{'checkbox-on':allSelect}" class="blue-checkbox-new" @click="selectAll"></span>
...
<script>
  import { getCartList, cartEdit, cartDel, delCartChecked } from '/api/goods'
  ...
  export default {
    ...
    computed: {
      ...
      checkPrice () {
        var totalPrice = 0
        this.cartList && this.cartList.forEach(item => {
          if (this.selectedArry.indexOf(item.goods.id) >= 0) {
            totalPrice += (item.nums * item.goods.price)
          }
        })
        return totalPrice
      },
      // 选中的商品数量
      checkNum () {
        return this.selectedArry.length
      }
    },
    methods: {
      ...mapMutations([
      ...
      selectAll () {
        if (this.allSelect) {
          ...
          this.cartList.forEach((item) => {
            if (this.selectedArry.indexOf(item.goods.id) < 0) {
              this.selectedArry.push(item.goods.id)
            }
          })
        }
      ...
      // 勾选商品
      selected (id) {
        let idIndex = this.selectedArry.indexOf(id)
        if (idIndex >= 0) {
            this.selectedArry.splice(idIndex, 1)
        } else {
          this.selectedArry.push(id)
        }
        if (this.selectedArry.length !== this.cartList.length) {
          this.allSelect = false;
        } else {
          this.allSelect = true;
        }
      },
      EditNum (goodsId, goodsNums) { // 数量
        this._cartEdit(goodsId, goodsNums)
      },
      // 删除整条购物车
      cartdel (goodsId) {
        cartDel({goods_id: goodsId}).then(res => {
